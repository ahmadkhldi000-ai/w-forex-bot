...
export function Footer() {
  return (
    <footer className="relative border-t border-border bg-bg-elev/40">
      <Container className="py-16">
        <div className="grid grid-cols-2 gap-10 md:grid-cols-3 lg:grid-cols-6">
          {/* Brand */}
          <div className="col-span-2">
            <a href="#" className="flex items-center gap-2.5">
              <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-emerald-bright to-emerald-deep">
                <TrendingUp className="h-5 w-5 text-[#04130d]" strokeWidth={2.5} />
              </span>
              <span className="flex items-baseline gap-1">
                <span className="text-lg font-bold text-fg">W</span>
                <span className="text-lg font-bold text-gradient-emerald">Forex</span>
            ...
            <div className="mt-5 flex gap-2">
              {socials.map(({ icon: Icon, label }) => (
                <a
                  ...
                  href="#"
                  aria-label={label}
                  className="flex h-9 w-9 items-center justify-center rounded-lg border border-border bg-surface text-fg-muted transition-all duration-300 hover:-translate-y-0.5 hover:border-border-strong hover:text-emerald-bright"
                >
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-border pt-7 text-center sm:flex-row sm:text-right">
          <p className="text-xs text-fg-dim">
            © 2026 W Forex Bot. جميع الحقوق محفوظة.
          </p>
          <p className="max-w-2xl text-xs leading-relaxed text-fg-dim">
            تحذير المخاطر: ينطوي تداول العملات الأجنبية على مخاطر عالية وقد لا
            يكون مناسباً لجميع المستثمرين. الأداء السابق لا يضمن النتائج المستقبلية.
          </p>
        </div>
      </Container>
    </footer>
